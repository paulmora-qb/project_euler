...
function problem10()
    ...
    cleaned_string = split(replace(string, "\n" => " "), " ")
    matrix = transpose(reshape([parse(Int, x) for x in cleaned_string], (20, 20)))
    matrix_size = size(matrix)

    cutout_matrix_size = 4

    greatest_product = 0
    for i in 1:(matrix_size[1]-cutout_matrix_size+1)
        for j in 1:(matrix_size[2]-cutout_matrix_size+1)
            cutout_matrix = matrix[i:i+(cutout_matrix_size-1), j:j+(cutout_matrix_size-1)]
